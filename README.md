Neon Tic-Tac-Toe Game

A modern and visually immersive Tic-Tac-Toe web application built using HTML, CSS, and JavaScript.
This project features a futuristic neon UI, smooth animations, and interactive gameplay for an engaging user experience.

🚀 Live Preview
👉 https://akshaya0323.github.io/SCT_WD_03/

📌 Project Overview

This project is part of a Web Development Internship Task (SCT_WD_03).
It demonstrates the implementation of a classic game with a modern UI/UX approach, focusing on animations, responsiveness, and design aesthetics.

✨ Features
🎮 Game Features

   ✔️ Two-player mode
   ✔️ Turn-based gameplay
   ✔️ Win & draw detection
   ✔️ Restart functionality

🌌 UI / UX Features

   ✔️ Neon glow effects
   ✔️ Glassmorphism design
   ✔️ Sci-fi fonts (Orbitron + Poppins) 
   ✔️ Smooth transitions

💫 Animations

   ✔️ Floating particles background
   ✔️ Hover shimmer effects
   ✔️ Winning cell animation
   ✔️ Modal popup animation

📊 Scoreboard

   ✔️ Player X score
   ✔️ Player O score
   ✔️ Draw tracking

📱 Responsive Design

   ✔️ Mobile-friendly
   ✔️ Adaptive grid layout
   
🛠️ Tech Stack
    💻 HTML5
    🎨 CSS3 (Animations + Effects)
    ⚙️ JavaScript
    🔤 Google Fonts
    
📂 Folder Structure
📦 SCT_WD_03
 ┣ 📜 index.html
 ┣ 📜 style.css
 ┣ 📜 script.js
 ┗ 📜 README.md
 
▶️ Run Locally
git clone https://github.com/akshaya0323/SCT_WD_03.git
cd SCT_WD_03

Then open:
index.html

🧠 How It Works
   - 3×3 grid using CSS Grid
   -Players take turns (X & O)
   -JavaScript handles:
      *Game state
      *Win conditions
      *Draw logic
   -Winning cells animate + modal appears

🎨 UI Highlights
   🌌 Space-themed background
   🔷 Neon glow + shadows
   🧊 Frosted glass UI
   💡 Interactive buttons
   🔥 Smooth hover effects
   
🔮 Future Improvements
   🤖 AI opponent (Minimax)
   🔊 Sound effects
   🌐 Online multiplayer
   🎨 Theme switcher

